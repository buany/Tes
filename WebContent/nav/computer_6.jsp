	...
	<div id="main">
		...
		<p align="center">
				...
				<c:choose>
					...
					<c:otherwise>
						<a href="computer.rbook?page=1">首页</a>
						<a href="computer.rbook?page=${UI.curPage-1 }">上一页</a>
					</c:otherwise>
				</c:choose>
				<c:choose>
					<c:when test="${UI.curPage==UI.totalPages }">下一页 尾页</c:when>
					<c:otherwise>
					<a href="computer.rbook?page=${UI.curPage+1 }">下一页</a>
					<a href="computer.rbook?page=${UI.totalPages }">尾页</a>
				</c:otherwise>
			</c:choose>
		</p>
	</div>
